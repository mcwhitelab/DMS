...
```{r}
directory <- "/scratch/gpfs/cmcwhite/preds_alphafold/interaction_exp/fastas/"
filelist <- c("5ht1b_50U_gnao.fasta.attn_compare.bfd.gpu.csv","5ht1b.fasta.attn_compare.t5.gpu.csv",
"myc_50U_max.fasta.attn_compare.t5.gpu.csv",  "myc.fasta.attn_compare.t5.gpu.csv", "5ht1b.fasta.attn_compare.bfd.gpu.csv","myc_50U_max.fasta.attn_compare.bfd.gpu.csv", "myc.fasta.attn_compare.bfd.gpu.csv")
...
pos_zscores_int  %>% filter(refprot == "myc") %>% filter(model == "bfd") %>% 
  group_by(head, pos, model, refprot) %>%
  pivot_wider(values_from = "pos_z", names_from = "proteinID") 
